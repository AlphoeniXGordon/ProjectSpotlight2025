...
public class EventCenter
{
    ...
    private static void OnListenerRemoving(EventDefine eventType, Delegate callBack)
    {
        ...
        {
            //Debug.Log(string.Format("移除监听错误：没有事件码{0}", eventType));
        }
    }
    private static void OnListenerRemoved(EventDefine eventType)
    {
        if (m_EventTable.ContainsKey(eventType)&& m_EventTable[eventType] == null)
        {
    ...
    //two parameters ref
    public static void Broadcast<T, X>(EventDefine eventType, T arg1,ref X arg2)
    {
        Delegate d;
        if (m_EventTable.TryGetValue(eventType, out d))
        {
